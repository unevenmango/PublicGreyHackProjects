...
Reshandle = "get_shell.host_computer.touch("+q+"home_dir"+q+","+q+"PassOutput.txt"+q+")\npassfile = get_shell.host_computer.File("+q+"home_dir"+q+"+"+q+"/PassOutput.txt"+q+")\nfor file in search.passFileList\nprint(""saveing : ""+file.name )\n"
Reshandle = Reshandle + "if not file.hasIndex(""content"") then continue\npassfile.set_content(passfile.content +"+q+"##################"+q+"+ file.name +"+q+"##################"+q+"+ file.content)\n"
...
NetExploit = function()
    ...
    for freebe in NOREQ
        ...
        Exploit = Exploit + "result = metalib.overflow("+q+nameList[0]+q+", "+q+nameList[1]+q+rout+")\nif params.len == 3 then result = metalib.overflow("+q+nameList[0]+q+", "+q+nameList[1]+q+rout+", params[2])\nif not result then exit("+q+"failed"+q+")\nshell=null\nprint("+q+"<b>Revieved: "+q+"+typeof(result))\n"
        Exploit = Exploit + "if typeof(result) == "+q+"computer"+q+" then search.Auto(result)\nif typeof(result) == "+q+"file"+q+" then search.Auto(result)\nif typeof(result) == "+q+"shell"+q+" then\nshell = result\nsearch.Auto(shell.host_computer)\nend if\n"
...
modules.libpeter = {"help":"","var":{},"func":@libpeter,"classID":"/SimpleGenLocal~> "}
module = modules.base
